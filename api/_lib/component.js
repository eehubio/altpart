  ...
  try {
    ...
    referenceParams.forEach((ref, i) => {
      ...
      parameters[ref.id] = {
        ...
        source: "ai_lookup", sourceLabel: "AI查询",
        confidence: value === "N/A" ? "none" : "medium",
      };
    });
    console.log(`[AI] ${partNumber}: ${found}/${referenceParams.length} params`);
    return { partNumber: data.partNumber||partNumber, manufacturer: data.manufacturer||"", description: data.description||"", parameters, _source: "ai_lookup" };
